...
- **Factor Models Comparison Between Fama-French and Market**:  focusing on selecting relevant stocks by calculating and ranking these factors and assinging correponding weightings.
- **Sector Returns and Volatiltiy Comparison**: Evaluating the historical performance of these portfolios against both equally-weighted and market-cap-weighted S&P 500 indices.
- **Discussion of Abnormal Returns and Cumulative Returns**s: Analyzing the rolling 12-month annual return and rolling 12-month standard deviation among the portfolios compared to benchmarks.
- **Factor Exposures For Various Sectors**: Providing a detailed review of key performance indicators such as the Sharpe Ratio, Maximum Drawdown, Value at Risk (VaR), and Conditional Value at Risk (CVaR).
- **Event Windows Analysis**: Offering an overview of the sector distribution within the portfolios, highlighting predominant sectors in each strategy.  
